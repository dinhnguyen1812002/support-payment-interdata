...
const BlogCard: React.FC<BlogCardProps> = ({ posts: initialPosts = [], postCount }) => {
    const page = useTypedPage();
    const isAuthenticated = !!page.props.auth.user;
    const [posts, setPosts] = useState(initialPosts); // Quản lý posts bằng state

    const handleUpvoteChange = (postId: string, newUpvotes: number, newHasUpvoted: boolean) => {
        setPosts((prevPosts) =>
            prevPosts.map((post) =>
                post.id === postId
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {post.categories &&
                                                post.categories.map((category) => (
                                                    <Link key={category.id} href={`/categories/${generateSlug(category.title)}/posts`}>
                                                        <Badge
                                                            variant="outline"
                                                            className="px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 rounded border border-gray-200 border-dashed dark:border-gray-600 hover:border-blue-600 dark:hover:border-blue-400 dark:bg-[#0F1014]"
                                                        >
                                                            {category.title}
                                                        </Badge>
                                                    </Link>
